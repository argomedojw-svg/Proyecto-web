...
/* --- Todo el texto extraido, en un solo saco --- */
const trozos = [];
const empujarNodo = (n) => {
  trozos.push(n.texto);
...
for (const archivo of readdirSync(DIR)) {
  ...
  const j = JSON.parse(readFileSync(`${DIR}/${archivo}`, 'utf8'));
...
const contiene = (v) => saco.includes(v) || sacoSinGuion.includes(v.replace(/- /g, ''));

function presente(n) {
  for (const v of variantes(n)) {
    if (contiene(v)) return true;
  }
  ...
  if (dosPuntos > 0) {
    ...
    if (etiqueta.length >= 3 && contiene(etiqueta) && (descripcion.length < 12 || contiene(descripcion))) {
      return true;
    }
  }
...
for (let i = 0; i < lineas.length; i++) {
  ...
  if (/^<table/i.test(texto)) {
    ...
    for (const celda of celdas) {
      for (const parte of celda.split('\n')) {
        ...
        else ausentes.push({ linea: i + 1, texto: parte.trim().slice(0, 120) });
      }
    }
    continue;
  }
  ...
  if (ok) cubiertas += 1;
  else ausentes.push({ linea: i + 1, texto: cruda.slice(0, 140) });
}
...
for (const a of ausentes.slice(0, 20)) console.log(`  ✗ línea ${a.linea}: ${a.texto}`);
